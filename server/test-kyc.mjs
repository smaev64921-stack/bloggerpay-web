/* Проверка личности перед выводом: заявка с фото, решения оператора.
...
const bad1 = await api('POST', '/api/kyc/submit', { name: 'Иван', birth: '01.01.1990', photo: PHOTO }, T1);
ok(bad1.status === 400, 'одно слово вместо ФИО отклонено', bad1.body);
const bad2 = await api('POST', '/api/kyc/submit', { name: 'Иванов Иван', birth: '1990-01-01', photo: PHOTO }, T1);
ok(bad2.status === 400, 'дата не в формате ДД.ММ.ГГГГ отклонена', bad2.body);
const bad3 = await api('POST', '/api/kyc/submit', { name: 'Иванов Иван', birth: '01.01.1990', photo: 'привет' }, T1);
ok(bad3.status === 400, 'не-картинка вместо фото отклонена', bad3.body);
const bad4 = await api('POST', '/api/kyc/submit', { name: 'Иванов Иван', birth: '01.01.1990', photo: PHOTO }, null);
ok(bad4.status === 401, 'без входа заявку не подать', bad4.body);

/* нормальная заявка встаёт в очередь */
const sub = await api('POST', '/api/kyc/submit', { name: 'Иванов Иван Иванович', birth: '01.01.1990', photo: PHOTO }, T1);
...
const re = await api('POST', '/api/kyc/submit', { name: 'Иванов Пётр Иванович', birth: '02.02.1992', photo: PHOTO }, T1);
...
ok(!!mine && mine.name === 'Иванов Пётр Иванович' && mine.photo === undefined && mine.has_photo === 1,
   'оператор видит заявку, фото в списке нет', mine && { name: mine.name, has_photo: mine.has_photo });
...
const again = await api('POST', '/api/kyc/submit', { name: 'Иванов Пётр Иванович', birth: '02.02.1992', photo: PHOTO }, T1);
...
const sub2 = await api('POST', '/api/kyc/submit', { name: 'Петров Пётр Петрович', birth: '03.03.1993', photo: PHOTO }, T2);
...
const sub3 = await api('POST', '/api/kyc/submit', { name: 'Петров Пётр Петрович', birth: '03.03.1993', photo: PHOTO }, T2);
